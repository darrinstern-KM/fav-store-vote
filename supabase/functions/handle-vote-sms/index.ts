...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Received SMS vote webhook');
    ...
    if (!match) {
      const responseMessage = `Invalid format. Please text: VOTE [Store Name] [City]\n\nExample: VOTE Michaels Chicago`;
      return new Response(
    ...
    console.log('Vote recorded successfully for store:', store.shop_name);
    ...
    console.error('Error processing SMS vote:', error);
